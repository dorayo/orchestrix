...
This task guides you through creating a new Orchestrix agent following the standard template.

## Prerequisites

- Agent template: `../templates/agent-tmpl.md`
- Target directory: `.orchestrix-core/agents/`
...
- If your agent will perform any actions → You MUST create corresponding tasks in `.orchestrix-core/tasks/`
- If your agent will create any documents → You MUST create templates in `.orchestrix-core/templates/` AND include the `create-doc` task

Determine:

- **Custom Commands**: Agent-specific commands beyond the defaults
- **Required Tasks**: Tasks from `.orchestrix-core/tasks/` the agent needs
...
- **Required Templates**: Templates from `.orchestrix-core/templates/` the agent uses
...
1. Copy the template from `.orchestrix-core/templates/agent-tmpl.md`
...
3. Save as `.orchestrix-core/agents/[agent-id].md`
...
✅ Agent Created: [agent-name]
   Location: .orchestrix-core/agents/[agent-id].md
...
1. **For Missing Tasks**: Create using `.orchestrix-core/templates/task-template.md`
