...
This task helps you create a sophisticated Orchestrix expansion pack with advanced agent orchestration, template systems, and quality assurance patterns based on proven best practices.

## Understanding Expansion Packs

Expansion packs extend Orchestrix with domain-specific capabilities using sophisticated AI agent orchestration patterns. They are self-contained packages that leverage:
...
Every expansion pack MUST include a custom Orchestrix orchestrator agent with sophisticated command systems and numbered options protocols.
...
3. **Character Consistency**: Every agent must have a persistent persona with name, communication style, and numbered options protocol similar to `expansion-packs/orchestrix-2d-phaser-game-dev/agents/game-designer.md`
...
- [ ] {pack-name}-orchestrator (REQUIRED: Custom Orchestrix orchestrator)
...
**FIRST PRIORITY**: Design the custom Orchestrix orchestrator with domain-appropriate theme:
...
- **Integration Points**: How it coordinates with core Orchestrix agents
...
files:
  ...
  tasks:
    # Core utilities (REQUIRED - copy from orchestrix-core)
...
# Data files users must provide (in their orchestrix-core/data/ directory)
required_user_data:
...
# Dependencies on core Orchestrix components
core_dependencies:
  agents:
...
post_install_message: |
  ...
  REQUIRED USER DATA FILES (place in orchestrix-core/data/):
  - {data-file}.{ext}: {description and format}
  - {data-file-2}.{ext}: {description and format}

  QUICK START:
  1. Add required data files to orchestrix-core/data/
...
1. **Prepare Data Files** (place in `orchestrix-core/data/`):
...
- **Location**: Place in `orchestrix-core/data/`
...
Required user data files (orchestrix-core/data/):
...
10. "How should it integrate with core Orchestrix agents?"
...
- **Orchestrator Required**: Every pack MUST have a custom Orchestrix orchestrator with sophisticated command system
