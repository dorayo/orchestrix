...
Verify SM agent permissions via `.orchestrix-core/data/story-status-transitions.yaml`:
...
Load `.orchestrix-core/core-config.yaml` and extract `devStoryLocation`.
...
**Execute**: `.orchestrix-core/tasks/make-decision.md` with `decision_type: sm-story-tier`
...
context:
  ...
  story_content:
    ...
    acceptance_criteria: { story_definition.acceptance_criteria }
  explicit_tier: { story_definition.tier or null }
```

**LLM Analysis** (per `llm_analysis` in decision file): Semantic analysis of story_content.

**Output**: `tier_decision` with `result`, `analysis`, `workflow`

**Routing**:

- `result in [trivial, simple]` → **DELEGATE** to `create-quick-story.md`, **EXIT**
- `result in [standard, complex]` → Store for Step 8, **Continue**
...
```
.orchestrix-core/tasks/util-load-architecture-context.md
```
...
```
.orchestrix-core/tasks/util-load-cumulative-context.md
```
...
Read: `.orchestrix-core/templates/story-tmpl.yaml`
