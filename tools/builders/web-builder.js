...
class WebBuilder {
  ...
  convertToWebPath(filePath, bundleRoot = 'orchestrix-core') {
    ...
    if (pathParts[0] === 'expansion-packs') {
      ...
      // For orchestrix-core, common, etc., remove the first part
  ...
  generateWebInstructions(bundleType, packName = null) {
    ...
    const rootExample = packName ? `.${packName}` : '.orchestrix-core';
    const examplePath = packName ? `.${packName}/folder/filename.md` : '.orchestrix-core/folder/filename.md';
    const personasExample = packName ? `.${packName}/personas/analyst.md` : '.orchestrix-core/personas/analyst.md';
    const tasksExample = packName ? `.${packName}/tasks/create-story.md` : '.orchestrix-core/tasks/create-story.md';
    const utilsExample = packName ? `.${packName}/utils/template-format.md` : '.orchestrix-core/utils/template-format.md';
    const tasksRef = packName ? `.${packName}/tasks/create-story.md` : '.orchestrix-core/tasks/create-story.md';

    return `# Web Agent Bundle Instructions

You are now operating as a specialized AI agent from the Orchestrix framework. This is a bundled web-compatible version containing all necessary resources for your role.
...
4. **Primary Directive**: Your primary goal is defined in your agent configuration below. Focus on fulfilling your designated role according to the Orchestrix framework.
...
`;
  ...
  async buildAgentBundle(agentId) {
    ...
    // Add agent configuration
          const agentPath = this.convertToWebPath(dependencies.agent.path, 'orchestrix-core');
      sections.push(this.formatSection(agentPath, dependencies.agent.content, 'orchestrix-core'));

    // Add all dependencies
    for (const resource of dependencies.resources) {
      const resourcePath = this.convertToWebPath(resource.path, 'orchestrix-core');
      sections.push(this.formatSection(resourcePath, resource.content, 'orchestrix-core'));
    }
  ...
  async buildTeamBundle(teamId) {
    ...
    const teamPath = this.convertToWebPath(dependencies.team.path, 'orchestrix-core');
    sections.push(this.formatSection(teamPath, dependencies.team.content, 'orchestrix-core'));

    // Add all agents
    for (const agent of dependencies.agents) {
      const agentPath = this.convertToWebPath(agent.path, 'orchestrix-core');
      sections.push(this.formatSection(agentPath, agent.content, 'orchestrix-core'));
    }
  ...
  formatSection(path, content, bundleRoot = 'orchestrix-core') {
    const separator = "====================";
  ...
  async buildExpansionTeamBundle(packName, packDir, teamConfigPath) {
    ...
    // Ensure orchestrix-orchestrator is always included for teams
    if (!agentsToProcess.includes("orchestrix-orchestrator")) {
      console.warn(`    ⚠ Team ${teamFileName} missing orchestrix-orchestrator, adding automatically`);
      agentsToProcess.unshift("orchestrix-orchestrator");
    }
    ...
    for (const [key, dep] of allDependencies) {
      ...
      if (!found) {
        const corePath = path.join(this.rootDir, "orchestrix-core", dep.type, dep.name);
        try {
