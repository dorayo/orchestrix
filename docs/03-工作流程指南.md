...
4. **执行两阶段质量评估**：
   - **阶段一：结构验证**（必须100%通过）
     - 验证模板完整性、AC-Task 映射、逻辑顺序等
     - 如果失败：Status = `Blocked`，SM 需要修订
   - **阶段二：技术质量评估**（仅在结构验证通过后）
     - 技术提取完成率（必须 ≥80%）
     - 技术质量评分（0-10分）
     - 复杂度指标检测（0-7个指标）
5. **智能决策矩阵应用**：
   - 高质量（≥8.0）+ 低复杂度（0）→ Status = `Approved`
   - 高质量（≥8.0）+ 高复杂度（≥2）→ Status = `AwaitingArchReview`
   - 中等质量（6.0-7.9）→ Status = `AwaitingArchReview`
   - 低质量（<6.0）→ Status = `Blocked`
6. **生成用户故事**：检查 `docs/stories/` 目录

**🔄 SM → Architect 交接点**：

- **触发条件**：Story Status = `AwaitingArchReview`
- **Handoff 消息**：`Next: Architect 请执行命令 'review-story [story-id]'`
- **交接内容**：Story 文件、质量评估元数据、复杂度指标

**🔄 SM → Dev 交接点**（直接批准场景）：

- **触发条件**：Story Status = `Approved`（高质量 + 低复杂度）
- **Handoff 消息**：`Next: Dev 请执行命令 'implement-story [story-id]'`
- **交接内容**：已批准的 Story 文件
...
4. **技术准确性评分**（0-10分）：
   ...
   - 技术可行性（1分）
5. **审核结果处理**：
   - **评分 ≥7 且无 Critical 问题**：Status = `Approved`
   - **评分 <7 或有 Critical 问题**：Status = `RequiresRevision`
   - **需要人工介入**：Status = `Escalated`
6. **审查轮次限制**：最多2轮审查，避免无限循环

**🔄 Architect → Dev 交接点**：

- **触发条件**：Story Status = `Approved`
- **Handoff 消息**：`Next: Dev 请执行命令 'implement-story [story-id]'`
...
- **Handoff 消息**：`Next: SM 请执行命令 'revise [story-id]'`
- **交接内容**：审查反馈、问题列表、修订建议
...
1. **开始新的IDE对话**（或继续原对话）
2. **加载 SM 代理**：`@sm`
3. **执行修订任务**：`*revise <story_id>`
4. **读取 Architect 反馈**：自动提取 Decision、Key Concerns、Recommendations
5. **修订 Story**：
   - 更新 Dev Notes
   - 调整 Tasks（如需要）
   - 更新架构引用
6. **重新质量检查**：执行结构验证、技术质量评分、复杂度检测
7. **智能决策逻辑**：
   - **自动批准条件**（满足所有条件）：
     - 所有 Critical 问题已解决
     - 质量分数提升 ≥2 分
     - 最终分数 ≥8.0
     - 仅有 Minor 问题修订
     - → Status = `Approved`（跳过第2轮审查）
   - **触发第2轮审查**：
     - 有 Critical 问题修订
     - → Status = `AwaitingArchReview`（第2轮）
   - **询问用户**：
     - 有 Medium 问题修订且分数 ≥7.0
     - → 用户决定是否触发第2轮审查
...
- **Handoff 消息**：`Next: Architect 请执行命令 'review-story [story-id]' (第2轮审查)`
...
1. **开始新的IDE对话**
2. **加载 Dev 代理**：`@dev`
3. **选择要实现的故事**：`*implement-story <story_id>`
4. **执行开发任务**：
   - 按照 Story 的 Tasks 顺序实现
   - 遵循 Dev Notes 中的技术指导
   - 编写测试（如 Story 要求）
5. **完成实现**：
   - 更新 Dev Agent Record
...
- **Handoff 消息**：`Next: QA 请执行命令 'review [story-id]'`
- **交接内容**：实现的代码、测试、Dev Agent Record
...
4. **渐进式审查标准**（最多3轮）：
   ...
   - **第3轮**：务实标准
     ...
     - → 仍有问题: 询问用户决策
5. **生成 QA Gate 文件**：`docs/qa/gates/[story-id]-gate-[round].md`

**🔄 QA → Dev 交接点**：

- **触发条件**：Story Status = `InProgress`（QA 发现问题）
- **Handoff 消息**：`Next: Dev 请执行命令 'review-qa [story-id]'`
- **交接内容**：QA Gate 文件、问题列表、修复建议

**🔄 QA → 完成**：

- **触发条件**：Story Status = `Done`
- **Handoff 消息**：`Story 已完成！`
...
1. **继续或开始新的IDE对话**
2. **加载 Dev 代理**：`@dev`
3. **执行修复任务**：`*review-qa <story_id>`
4. **读取 QA Gate 文件**：自动提取问题列表和优先级
5. **修复问题**：
   - 按优先级修复（High → Medium → Low）
   - 更新代码和测试
   - 记录修复内容
6. **完成修复**：Status = `Review`（触发再次 QA）
