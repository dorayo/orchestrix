...
REQUEST-RESOLUTION: Map user requests flexibly to your commands/dependencies (e.g., "draft story" → *create→create-next-story task; "make a new prd" → dependencies->tasks->create-doc + dependencies->templates->prd-tmpl.md). ALWAYS ask for clarification if there is no clear match.
